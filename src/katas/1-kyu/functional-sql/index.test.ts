...
describe("SQL tests", function () {
  ...
  it("Basic SELECT and WHERE over objects", () => {
    ...
    expect(query().select(profession).from(persons).where(isTeacher).execute()).to.deep.equal([
      "teacher",
      "teacher",
      "teacher",
    ]);
    ...
    expect(query().select(name).from(persons).where(isTeacher).execute()).to.deep.equal([
      "Peter",
      "Michael",
      "Peter",
    ]);
    expect(query().where(isTeacher).from(persons).select(name).execute()).to.deep.equal([
      "Peter",
      "Michael",
      "Peter",
    ]);
  ...
  it("Number tests", () => {
    ...
    expect(query().select().from(numbers).orderBy(descendentCompare).execute()).to.deep.equal([
      9, 8, 7, 6, 5, 4, 3, 2, 1,
    ]);
